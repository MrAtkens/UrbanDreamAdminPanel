...
const CustomerUpdateForm: React.FC<Props> = observer(() => {
  ...
  const { register, errors, handleSubmit } = useForm({
    mode: 'onChange',
    defaultValues: customers.getCustomerById
  });
  const [checked, setChecked] = React.useState(false);
  const onSubmit = (data) => {
    console.log(data)
    customers.editCustomer(data.firstName, data.lastName, data.phoneNumber, data.password, data.email).then(() => {
      closeDrawer();
    })
  };
